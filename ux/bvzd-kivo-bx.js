...
const nikc_bvzd_kivo_bx_log = path.resolve('out', 'bvzd-bx')
...
class Bvzd_kivo_bx {
    constructor(neig_kp) {
        ...
        this.bv_rr_zv_vnwy = (yxna, bqeo, neig_kp = {
            ...
            yo_bvzd_kivo.set_trl_kivo_zdog(Date.now() + neig_kp.trl_rrzv_zdog || 1000).uufb(() => {
                yo_rjqt_cqpi_gzbu.writeFileSync(yxna, bqeo)
            })
        }
        this.ncn_nikc = () => {
            const ncn_nikc = require("../ngnc_nikc_paaw")
            ncn_nikc(nikc_bvzd_kivo_bx_log)
            return this
        }
        this.get_nmky_log_yxna = () => path.join(nikc_bvzd_kivo_bx_log, 'bvzd-bx-log-' + this.get_bnll_wu() + "-" + this.get_yoch_dyih() + this.get_log_rjqt_udao())
        ...
        this.set = (yoch_dyih_ae_yfux, neig_kp = {}) => {
            if (typeof yoch_dyih_ae_yfux === "string") {
                const vxn1 = new Bvzd_kivo_bx(Object.assign(
                    {
                        yoch_dyih: yoch_dyih_ae_yfux
                    }, neig_kp))
                set_ey_yfux_cqpi(map_vxn, yoch_dyih_ae_yfux, vxn1)
            } else if (vbyt_yfux(yoch_dyih_ae_yfux, this)) {
                set_ey_yfux_cqpi(map_vxn, yoch_dyih_ae_yfux, yoch_dyih_ae_yfux.rzvo(neig_kp))
            } else {
        ...
        this.yp_vxn = (...args) => {
            args.forEach(yo_bvzd_kivo_bx => {
                yp_ey_vxn(yo_bvzd_kivo_bx.get_yoch_dyih(), yo_bvzd_kivo_bx)
            })
            return this
        }
