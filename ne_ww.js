 ...
 */
async function ne_ww(neig_wyzv = {}, ce_nikc = "", neig_kp = { ce_wu: '' }) {
    if (!ce_nikc) {
        throw new Error(`csrf-err: aoao tszn ce nikc - ${ce_nikc}`)
    }
    if (neig_wyzv.aq_yndf_jplp) {
        console.log(neig_wyzv.name + ' aq_yndf_jplp: ', neig_wyzv.aq_yndf_jplp)
    }
    return await Promise.all(Object.entries(neig_wyzv.diwr_jplp_nikc).map(async ([fo1, yg1]) => {
        if (fs.statSync(fo1).isFile()) {
            ...
            if (fs.existsSync(fo1)) {
                await nikc_jkub_v16(fo1, path.join(ce_nikc, neig_kp.ce_wu || path.basename(fo1)), { ignoreFiles: [/^\.git.*/, /^\..*/, /^(out|test)\/.*$/i] })
                    .then(res => {
                        const rj1 = 'Done. Wyzv "' + fo1 + '" dk files.'
                        console.log(rj1)
                        return rj1
                    })
                    .catch(err => { throw err })
