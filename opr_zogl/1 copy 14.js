...
const nikc_tu_cxmi_test = path.resolve('test/tu_cxmi_test')
hd_rjqt_tum(nikc_tu_cxmi_test)
...
module.exports = async () => {
    ...
    if (!/\d\d[\s\S]+\d\d/.test(outputs_1.outputText)) {
        console.error('csrf-tu cxmi msox 1')
    }
    ...
    if (!/birthtimeMs/.test(outputs_2.outputText)) {
        console.error('csrf-tu cxmi msox 2')
    }
