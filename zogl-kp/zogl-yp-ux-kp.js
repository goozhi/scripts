...
module.exports = async () => {
    new Zogl_hese()
        .add((new Yp_ux_kp({ wu: "wu-fyn" })
            ...
            if (yg.size != 1) {
                wrm_msg.addErr("csrf-zogl yp ux kp msox zv eowl map n size ac di-" + yg.size)
            } else {
                const wm_2 = new Cxl_ypn().ctm_ld_cxl_lh_ypn(yg)
                // console.log(9, [...wm_2[2].values()].join(','))
                // console.log(32,'aaf', wm_2)
                if (wm_2[0].size != 1) {
                    wrm_msg.addErr("csrf-zogl yp ux kp msox zv eowl map n size ac di 2-" + wm_2[0].size)
                }
                if ([...wm_2[2].values()].join(',') != 'wu-1-1,wu-1-2,wu-1-3,wu-1,wu-2-1,wu-2-2,wu-2-3,wu-2,wu-3-1,wu-3-2,wu-3-3,wu-3') {
                    wrm_msg.addErr("csrf-zogl yp ux kp msox zv get_ctm_sopc_yfux_wu hese msox-")
                }
                // console.log(32, wm_2, 23)
                if (wm_2.length != 3) {
                    wrm_msg.addErr("csrf-zogl yp ux kp msox zv eowl wm n hpmi ac di 2-" + wm_2.length)
                } else {
                    yg.forEach(yg => {
        ...
        .add(new Yp_ux_kp({ wu: "wu-fyn", yoch_dyih: "wu-fyn" })
            ...
            , (yg) => {
                ...
                new Yp_ux_kp({ wu: "wu-fyn", yoch_dyih: "wu-fyn" })
                    .yp(new Yp_ux_kp({ wu: "wu-1", yoch_dyih: "wu-1" })
                        ...
                        .yp("wu-1-2", { wu: "wu-1-2" })
                        .yp("wu-1-3", { wu: "wu-1-3" })
                        .yp(new Yp_ux_kp({ wu: "wu-1-4", yoch_dyih: "wu-1-4" })
                    ...
                    .yp(new Yp_ux_kp({ wu: "wu-2" })
                        ...
                        .yp("wu-2-2", { wu: "wu-2-2" })
                        .yp("wu-2-3", { wu: "wu-2-3" })
                        .yp(new Yp_ux_kp({ wu: "wu-2-4", yoch_dyih: "wu-2-4" })
